...
async function _flushSave(){
  ...
  if(saveInFlight){ savePending=true; return; } // another save is mid-flight; mark for re-run
  saveInFlight=true;
  ...
  try{
    ...
    saveInFlight=false;
    if(savePending){ savePending=false; _flushSave(); } // state changed again while we were saving — flush latest
  }
}
function flushSaveNow(){ _flushSave(); }
function saveState(){
  if(LOCAL_MODE){ localStorage.setItem(localKey(),JSON.stringify(STATE)); return; }
  // Debounce: coalesce rapid-fire calls (e.g. answering many questions quickly)
  clearTimeout(saveTimer);
  saveTimer=setTimeout(_flushSave,600);
  // Safety net: guarantee a flush at least every 2s even during continuous bursts,
  // so a long rapid-answering session can never fully suppress saving to Supabase.
  if(!saveMaxWaitTimer){
    saveMaxWaitTimer=setTimeout(_flushSave,2000);
  }
}
// Last-resort safety: flush any pending save if the user navigates away or closes the tab/app
window.addEventListener("visibilitychange",()=>{ if(document.visibilityState==="hidden") flushSaveNow(); });
