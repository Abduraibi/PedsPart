...
// Exam date is now stored per-user in STATE.examDate
// These getters fall back to the original dates for existing users
function getExamDate(){ return (STATE&&STATE.examDate)||"2026-07-07"; }
function getFinishBy(){
  const d=new Date(getExamDate()+"T12:00:00");
...
async function onLogin(user){
  ...
  // Inject settings button next to logout button if not already present
  if(!document.getElementById("settingsBtn")){
    ...
    if(logoutBtn){
      ...
      logoutBtn.parentNode.insertBefore(sb, logoutBtn);
    }
  }
  await loadState();
...
/* One-time migration: existing users (who have seen questions) get the
   hardcoded dates and fixed 35-day schedule preserved automatically */
function migrateExistingUser(){
  if(!STATE) return;
  // Already migrated
  if(STATE.examDate) return;
  // Has existing progress = existing user → give them the hardcoded schedule
  ...
  // New users with no progress: examDate stays null → triggers onboarding
}
...
/* ---------------------------------------------------------------------------
   ONBOARDING — shown to new users who have no examDate set
--------------------------------------------------------------------------- */
function showOnboarding(){
  const existing = document.getElementById('onboardingBackdrop');
  if(existing) existing.remove();
  const today = todayStr();
  // Default exam date: 35 days from today
  const def = new Date(today+'T12:00:00');
  def.setDate(def.getDate()+35);
  const defStr = def.toISOString().slice(0,10);

  const bd = document.createElement('div');
  bd.className = 'modal-backdrop';
  bd.id = 'onboardingBackdrop';
  ...
  bd.innerHTML = `
    <div class="modal" style="max-width:480px;text-align:center">
      ...
      <p style="color:var(--ink-soft);font-size:14.5px;margin-bottom:24px">
        Set your exam date and the daily plan will automatically adapt — balancing new questions,
        memory review, and specialty focus to get you ready in time.
      </p>
      ...
      <div id="onboardPreview" style="background:var(--bg);border-radius:10px;padding:12px 16px;margin-bottom:20px;font-size:13.5px;color:var(--ink-soft);text-align:left"></div>
  ...
  const updatePreview = () => {
    const val = inp.value;
    if(!val){ document.getElementById('onboardPreview').textContent=''; return; }
    const examD = new Date(val+'T12:00:00');
    const finishD = new Date(val+'T12:00:00'); finishD.setDate(finishD.getDate()-1);
    const totalDays = Math.max(1, Math.round((examD - new Date(today+'T12:00:00'))/86400000));
    const finStr = finishD.toISOString().slice(0,10);
    document.getElementById('onboardPreview').innerHTML =
      `<b>${totalDays} study days</b> · plan finishes by <b>${finStr}</b><br>
       <span style="color:var(--green)">~${Math.ceil(QUESTIONS.filter(q=>!isMastered(q.id)).length/totalDays)} new questions/day</span>`;
  };
...
async function saveOnboarding(){
  ...
  const today = todayStr();
  if(val <= today){ toast("Exam date must be in the future."); return; }
  STATE.examDate  = val;
...
/* ---------------------------------------------------------------------------
   SETTINGS — exam date change anytime
--------------------------------------------------------------------------- */
function openSettings(){
  ...
  const curPlanStart = getPlanStart();
  const totalDays = getTotalPlanDays();
  const dayNum = daysBetween(curPlanStart, today) + 1;
  ...
  bd.innerHTML = `
    <div class="modal" style="max-width:480px">
      ...
      <div style="margin-bottom:22px">
        ...
        <div style="background:var(--bg);border-radius:10px;padding:14px 16px;margin-bottom:12px;font-size:13.5px">
          <div style="display:flex;justify-content:space-between;margin-bottom:5px">
            <span style="color:var(--ink-soft)">Current exam date</span>
            <b>${curExam}</b>
          </div>
          <div style="display:flex;justify-content:space-between;margin-bottom:5px">
            <span style="color:var(--ink-soft)">Plan started</span>
            <b>${curPlanStart}</b>
          </div>
          <div style="display:flex;justify-content:space-between;margin-bottom:5px">
            <span style="color:var(--ink-soft)">Total plan days</span>
            <b>${totalDays}</b>
          </div>
          <div style="display:flex;justify-content:space-between">
            <span style="color:var(--ink-soft)">Today is day</span>
            <b>${Math.min(dayNum, totalDays)} of ${totalDays}</b>
          </div>
        </div>
        ...
        <div id="settingsPreview" style="font-size:13px;color:var(--ink-soft);min-height:18px;margin-bottom:16px"></div>
        ...
        <div style="background:color-mix(in srgb,var(--yellow) 12%,transparent);border:1px solid color-mix(in srgb,var(--yellow) 30%,transparent);border-radius:9px;padding:11px 14px;font-size:13px;color:var(--ink-soft);margin-bottom:18px">
          ⚠️ Changing your exam date recalculates the daily quota and pace — but <b>never</b> touches your seen questions, mastered items, SRS intervals, wrong loop, or Fudul progress.
        </div>
  ...
  const updatePreview = () => {
    const val = inp.value;
    if(!val){ document.getElementById('settingsPreview').textContent=''; return; }
    const examD = new Date(val+'T12:00:00');
    const finD   = new Date(val+'T12:00:00'); finD.setDate(finD.getDate()-1);
    const newTotal = Math.max(1, Math.round((examD - new Date(curPlanStart+'T12:00:00'))/86400000));
    const newLeft  = Math.max(1, Math.round((examD - new Date(today+'T12:00:00'))/86400000));
    const unseenLeft = QUESTIONS.filter(q=>!STATE.seen[q.id]).length;
    document.getElementById('settingsPreview').innerHTML =
      `${newTotal} total days · <b>${newLeft} days remaining</b> · finish by ${finD.toISOString().slice(0,10)}<br>
       <span style="color:var(--green)">~${Math.ceil(unseenLeft/newLeft)} new questions/day</span>`;
  };
  inp.addEventListener('input', updatePreview);
  updatePreview();
}
...
async function saveSettings(){
  ...
  const today = todayStr();
  if(val <= today){ toast("Exam date must be in the future."); return; }
  const oldExam = STATE.examDate;
  STATE.examDate = val;
  STATE.useFixedSchedule = false; // switch to dynamic after manual change
  // Rebuild today's plan with new pacing (but keep done progress)
  const todayLog = STATE.dayLog[today];
  const preservedDone = todayLog?.done || {};
  delete STATE.dayLog[today];
  saveState();
  buildDayPlan(currentPlanMode||"full");
  // Restore done ticks
...
function getTodayScheduleEntry(){
  const dayNum = daysBetween(getPlanStart(), todayStr()) + 1; // 1-indexed
  const totalDays = getTotalPlanDays();
  const clamped = Math.max(1, Math.min(dayNum, totalDays));

  if(STATE && STATE.useFixedSchedule){
    // Existing users: use the hardcoded 35-day schedule
    const clampedFixed = Math.max(1, Math.min(dayNum, STUDY_SCHEDULE.length));
    return STUDY_SCHEDULE.find(e => e.day === clampedFixed) || STUDY_SCHEDULE[STUDY_SCHEDULE.length - 1];
  }

  // New users: generate a dynamic schedule entry based on day number
  // Distribute specialties proportionally by question count
  const specWeights = SPECIALTIES.map(sp => ({
    spec: sp,
    count: QUESTIONS.filter(q => q.specialty === sp).length
  }));
  const totalQs = specWeights.reduce((a, b) => a + b.count, 0);
  // Assign each day a specialty proportional to question count
  let bucket = 0;
  let assignedSpec = SPECIALTIES[0];
  for(const sw of specWeights){
    bucket += Math.round((sw.count / totalQs) * totalDays);
    if(clamped <= bucket){ assignedSpec = sw.spec; break; }
  }
  // Final days: General Pediatrics review
  if(clamped > totalDays - Math.max(2, Math.round(totalDays * 0.1))){
    assignedSpec = "General Pediatrics";
  }
  return { day: clamped, spec: assignedSpec };
}
