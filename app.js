...
function buildDayPlan(mode){
  const day=todayStr();
  // Reuse today's frozen plan ONLY if it was built today and is not archived
  const saved=STATE.dayLog[day];
  if(saved && saved.mode===mode && saved.groups && !saved.archived && saved.day===day){
    const groups=saved.groups.map(g=>({
      key:g.key,title:g.title,sub:g.sub,icon:g.icon,
      items:g.itemIds.map(id=>QBY[id]).filter(Boolean)
    }));
    return {day,mode,groups,fudul:saved.fudul||fudulSessionForToday(),
            used:saved.used||[],done:saved.done||{}};
  }
  ...
  const sr=dueForReview().slice(0,mode==="busy"?12:25);
  ...
  const wl=pendingWrongLoop().filter(q=>!used.includes(q.id)).slice(0,mode==="busy"?15:30);
  ...
  if(mode==="busy"){
    // Busy day: 10-20 red zone (mix of new + seen-unmastered)
    const rz=pickZone("red",16,used);
    rz.forEach(q=>used.push(q.id));
    groups.push({key:"red",title:"Red Zone (light)",sub:"High-frequency must-know — 10–20 today",icon:"🔴",items:rz});
  }else{
  ...
  // FREEZE this plan — store day explicitly for date-guard check on next load
  STATE.dayLog[day]={
    day, mode, fudul, used,
  ...
  saveState();
  return {day,mode,groups,fudul,used,done:STATE.dayLog[day].done};
}
