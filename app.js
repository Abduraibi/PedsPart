...
function fudulSessionForToday(){
  ...
  return {
    ...
    from, to, dailyTarget, doneSoFar, total: FUDUL_TOTAL, pct, doneToday, daysLeft
  };
}
function toggleFudulDone(){
  ...
  if(alreadyDoneToday){
    // unmark — subtract exactly the batch we added
    ...
    // use the number in the input if available, otherwise default to daily target
    const inputEl=document.getElementById('fudulInput');
    const customCount=inputEl?Math.max(0,Math.min(f.total-STATE.fudulDone,parseInt(inputEl.value)||f.dailyTarget)):f.dailyTarget;
    const batch=Math.min(customCount, f.total-STATE.fudulDone);
...
function saveFudulCount(val){
  // live-update the input without marking done — just store for when Mark done is tapped
  // also update the display label to reflect what will be recorded
  const n=Math.max(0,parseInt(val)||0);
  const f=fudulSessionForToday();
  const newTo=Math.min(f.total, (STATE.fudulDone||0)+n);
  // update the target label dynamically
